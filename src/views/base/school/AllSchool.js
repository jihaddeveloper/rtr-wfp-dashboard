...
const AllSchool = () => {
  ...
  const handleRowUpdateAllSchool = (newData, oldData, resolve) => {
    ...
    if (errorList.length < 1) {
      axios
        .patch('http://118.179.80.51:8080/api/v1/schools/' + newData.id, newData, {
          method: 'PATCH',
          mode: 'no-cors',
          headers: {
            Accept: 'application/json',
  ...
  const handleRowAddSchool = (newData, resolve) => {
    ...
    if (errorList.length < 1) {
      axios
        .post('http://118.179.80.51:8080/api/v1/schools/', newData, {
          method: 'POST',
          mode: 'no-cors',
          headers: {
            Accept: 'application/json',
  ...
  const handleRowDeleteSchool = (oldData, resolve) => {
    ...
    if (errorList.length < 1) {
      axios
        .delete('http://118.179.80.51:8080/api/v1/schools/' + oldData.id, {
          method: 'DELETE',
          mode: 'no-cors',
          headers: {
            Accept: 'application/json',
          ...
          <CCardBody>
            <CAccordion alwaysOpen>
              <CAccordionItem itemKey={1}>
                <CAccordionHeader>
                  <strong>School in Coxbazar-{allSchoolData.length}</strong>
                </CAccordionHeader>
                <CAccordionBody>
                  <MaterialTable
