...
const DISchoolMonitoringNrFO = () => {
  // data state to store the BCO API data. Its initial value is an empty array
  const [data, setData] = useState([])
  const [isLoading, setIsLoading] = useState(false)

  const [allOverallSchool, setAllOverallSchool] = useState([])
  const [allDIOverallSchool, setAllDIOverallSchool] = useState([])
  ...
  useEffect(() => {
    const call = async () => {
      ...
      await getAllDIOverallSchool(console.log('get di overall school called'))
    }
    call()
  }, [])
  // Using useEffect to call the API once mounted and set the data
  ...
  const getAllDIOverallSchool = async () => {
    try {
      ...
      setAllDIOverallSchool(response.data)
      setIsLoading(false)
      console.log('Data:' + response)
    } catch (error) {
      console.log(error)
  ...
  const AllDISchoolMonitoringNrFO = allDIOverallSchool.filter((item) => {
  ...
  // Row update function
  // Row add function
  const handleRowAddOverallSchool = (newData, resolve) => {
    //validation
    ...
    if (errorList.length < 1) {
      axios
        .post('http://118.179.80.51:8080/api/v1/overall-school/', newData, {
          method: 'POST',
          mode: 'no-cors',
          headers: {
            Accept: 'application/json',
        ...
        .then((res) => {
          const dataToAdd = [...allOverallSchool]
          dataToAdd.push(newData)
          setAllOverallSchool([...dataToAdd])
          resolve()
          setIserror(false)
          setErrorMessages([])
          // console.log('newData.id: ' + newData.id)
        ...
        .catch((error) => {
          setErrorMessages(['Add OverallSchool failed! Server error'])
          setIserror(true)
          resolve()
        })
    } else {
  ...
  // Row delete function
  const handleRowDeleteOverallSchool = (oldData, resolve) => {
    //validation
    ...
    if (errorList.length < 1) {
      axios
        .delete('http://118.179.80.51:8080/api/v1/overall-school/' + oldData.id, {
          method: 'DELETE',
          mode: 'no-cors',
          headers: {
            Accept: 'application/json',
        ...
        .then((res) => {
          const dataDelete = [...allOverallSchool]
          const index = oldData.tableData.id
          dataDelete.splice(index, 1)
          setAllOverallSchool([...dataDelete])
          resolve()
          setIserror(false)
          setErrorMessages([])
          // console.log('newData.id: ' + newData.id)
    ...
    <CRow>
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>ALL DI School Monitoring Data</strong>
            {/* <strong>{allBCOData.length}</strong> */}
          </CCardHeader>
          <CCardBody>
            <MaterialTable
              title={AllDISchoolMonitoringNrFO.length + ' Data'}
              columns={[
                {
                  title: 'date',
                  field: 'date',
              ...
              data={AllDISchoolMonitoringNrFO}
            />
          </CCardBody>
        </CCard>
      </CCol>
...
export default DISchoolMonitoringNrFO
